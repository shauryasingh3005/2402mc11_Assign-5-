...
The Dining Philosophers problem demonstrates process synchronization when multiple philosophers share limited resources. 5 Philosophers repeatedly think, become hungry, acquire two forks, eat and release the forks. In my implementation, semaphores control access to the forks, and resource ordering is used by always acquiring the lower-numbered fork first. This prevents deadlocks by eliminating circular wait. The program successfully allows all 5 philosophers to complete their eating cycles without getting permanently blocked.
